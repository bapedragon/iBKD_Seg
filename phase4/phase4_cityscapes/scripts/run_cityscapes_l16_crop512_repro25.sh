...
output_root="${CITYSCAPES_CROP512_REPRO_OUTPUT:-/app/output/cityscapes_l16_crop512_repro25_v4}"
config="${PWD}/phase4/phase4_cityscapes/configs/paper_l16_crop512_repro25_v4.json"
...
run_suite() {
  ...
  echo "[CROP512_L16_REPRO] stage=verify_zip"
  ...
  echo "[CROP512_L16_REPRO] stage=prepare_train_val"
  ...
  echo "[CROP512_L16_REPRO] stage=official_sources_and_pretrained_weights"
  python -m ibkd_seg.cityscapes.official_assets --cache-root "${cache_root}"
  echo "[CROP512_L16_REPRO] stage=exact_lg_lg_alg_audit"
