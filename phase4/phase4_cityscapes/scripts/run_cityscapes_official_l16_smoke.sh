#!/usr/bin/env bash
set -euo pipefail

zip_dir="${CITYSCAPES_ZIP_DIR:-/app/data/chaoyang}"
data_dir="${CITYSCAPES_OFFICIAL_DATA_DIR:-/app/scratch/cityscapes_official_l16_v2/cityscapes}"
cache_root="${CITYSCAPES_OFFICIAL_CACHE:-/app/scratch/cityscapes_official_l16_v2/upstream}"
output_root="${CITYSCAPES_OFFICIAL_OUTPUT:-/app/output/cityscapes_official_l16_smoke_v2}"
mkdir -p "${output_root}"

run_suite() {
  python -m pip install --disable-pip-version-check -e .
  echo "[OFFICIAL_L16_PIPELINE] stage=verify_zip"
  python phase4/phase4_cityscapes/scripts/check_cityscapes_upload.py \
    --search-root "${zip_dir}" --output "${output_root}/upload_check.json"
  echo "[OFFICIAL_L16_PIPELINE] stage=prepare_train_val"
  python -m ibkd_seg.cityscapes.prepare --zip-dir "${zip_dir}" --data-dir "${data_dir}"
  cp "${data_dir}/manifest.json" "${output_root}/manifest.json"
  cp "${data_dir}/preparation.json" "${output_root}/preparation.json"
  echo "[OFFICIAL_L16_PIPELINE] stage=official_sources_and_pretrained_weights"
  python -m ibkd_seg.cityscapes.official_assets --cache-root "${cache_root}"
  echo "[OFFICIAL_L16_PIPELINE] stage=three_step_smoke"
  python -u -m ibkd_seg.cityscapes.official_smoke --device cuda \
    --cache-root "${cache_root}" --data-dir "${data_dir}" \
    --manifest "${output_root}/manifest.json" --output-dir "${output_root}/artifacts"
}

run_suite 2>&1 | tee "${output_root}/run.log"
