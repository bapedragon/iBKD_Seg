...
output_root="${CITYSCAPES_CROP512_REPRO_FORENSICS_OUTPUT:-/app/output/cityscapes_l16_crop512_repro_forensics1_v8}"
config="${PWD}/phase4/phase4_cityscapes/configs/paper_l16_crop512_repro_forensics1_v8.json"
...
run_suite() {
  ...
  echo "[CROP512_L16_REPRO_FORENSICS] stage=verify_zip"
  ...
  echo "[CROP512_L16_REPRO_FORENSICS] stage=prepare_train_val"
  ...
  echo "[CROP512_L16_REPRO_FORENSICS] stage=official_sources_and_pretrained_weights"
  python -m ibkd_seg.cityscapes.official_assets --cache-root "${cache_root}"
  echo "[CROP512_L16_REPRO_FORENSICS] stage=vanilla_a_b_lg_a_b_one_batch"
  python -u -m ibkd_seg.cityscapes.reproducibility_forensics \
