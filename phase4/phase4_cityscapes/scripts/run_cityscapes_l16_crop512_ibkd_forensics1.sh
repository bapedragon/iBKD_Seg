...
output_root="${CITYSCAPES_IBKD_FORENSICS_OUTPUT:-/app/output/cityscapes_l16_crop512_ibkd_forensics1_v10}"
config="${PWD}/phase4/phase4_cityscapes/configs/paper_l16_crop512_ibkd_forensics1_v10.json"
...
run_suite() {
  ...
  echo "[CROP512_L16_IBKD_FORENSICS] stage=verify_zip"
  ...
  echo "[CROP512_L16_IBKD_FORENSICS] stage=prepare_train_val"
  ...
  echo "[CROP512_L16_IBKD_FORENSICS] stage=official_sources_and_pretrained_weights"
  python -m ibkd_seg.cityscapes.official_assets --cache-root "${cache_root}"
  echo "[CROP512_L16_IBKD_FORENSICS] stage=paired_component_backward_cases"
  python -u -m ibkd_seg.cityscapes.ibkd_reproducibility_forensics \
