...
output_root="${CITYSCAPES_CROP512_BETA_GRID_OUTPUT:-/app/output/cityscapes_l16_crop512_beta_grid500_v5}"
config="${PWD}/phase4/phase4_cityscapes/configs/paper_l16_crop512_beta_grid500_v5.json"
mkdir -p "${output_root}"

run_suite() {
  python -m pip install --disable-pip-version-check -e .
  echo "[CROP512_L16_BETA_GRID] stage=verify_zip"
  ...
  echo "[CROP512_L16_BETA_GRID] stage=prepare_train_val"
  ...
  echo "[CROP512_L16_BETA_GRID] stage=official_sources_and_pretrained_weights"
  python -m ibkd_seg.cityscapes.official_assets --cache-root "${cache_root}"
  echo "[CROP512_L16_BETA_GRID] stage=12_runs_500_steps"
