...
output_root="${CITYSCAPES_CROP512_OUTPUT:-/app/output/cityscapes_l16_crop512_smoke_v3}"
config="${PWD}/phase4/phase4_cityscapes/configs/paper_l16_crop512_smoke_v3.json"
mkdir -p "${output_root}"

run_suite() {
  python -m pip install --disable-pip-version-check -e .
  echo "[CROP512_L16_PIPELINE] stage=verify_zip"
  ...
  echo "[CROP512_L16_PIPELINE] stage=prepare_train_val"
  ...
  echo "[CROP512_L16_PIPELINE] stage=official_sources_and_pretrained_weights"
  python -m ibkd_seg.cityscapes.official_assets --cache-root "${cache_root}"
  echo "[CROP512_L16_PIPELINE] stage=three_step_smoke"
  python -u -m ibkd_seg.cityscapes.official_smoke --device cuda \
