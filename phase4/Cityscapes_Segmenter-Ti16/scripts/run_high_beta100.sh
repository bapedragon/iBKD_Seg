...
cd "${repo_root}"
if [[ "$#" -ne 0 ]]; then
  echo 'This fixed six-run 100-step probe accepts no arguments' >&2
  exit 2
fi
export PYTHONPATH="${repo_root}/src${PYTHONPATH:+:${PYTHONPATH}}"
export CUBLAS_WORKSPACE_CONFIG=:4096:8
export CITYSCAPES_TI16_JOB_STARTED="$(date +%s)"
...
output_root="${CITYSCAPES_TI16_OUTPUT:-/app/output/cityscapes_ti16_high_beta100_v1/run_$(date -u +%Y%m%dT%H%M%SZ)_$$}"
config="${repo_root}/phase4/Cityscapes_Segmenter-Ti16/configs/high_beta100_v1.json"
mkdir -p "${output_root}"
report_failure() {
  local code="$?"
  if [[ "${code}" -ne 0 ]]; then
...
timeout --signal=TERM --kill-after=90s 35880s bash -c '
  ...
  echo "[TI16_HIGH_BETA100_START] runs=6 steps_each=100 seed=1 beta2p5_and_classification_ratio output=${output_root}"
