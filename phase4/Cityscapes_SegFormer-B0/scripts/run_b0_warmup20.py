#!/usr/bin/env python3
"""iBKD 두 λ의 warm-up20·β4개·10k 재선별과 재개를 관리한다."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    def stage(name):
        report['stage']=name;save();print('[B0_W20] '+json.dumps(dict(pack=args.pack,stage=name)),flush=True)
    ...
    try:
        ...
        stage('unit_checks')
        run([sys.executable,'-m','pytest','-q','tests/test_cityscapes_b0_training.py','tests/test_cityscapes_b0_calibration.py','tests/test_cityscapes_b0_warmup20.py'])
        stage('data')
        ...
        stage('assets')
        report['assets']=prepare_assets(cache,weight_names=('cirkd_teacher.pth','nvidia_mit_b0.bin'),manifest_name='warmup20_asset_manifest.json')
