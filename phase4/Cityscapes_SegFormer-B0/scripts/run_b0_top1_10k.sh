#!/usr/bin/env bash
set -euo pipefail
if [[ $# -gt 1 ]]; then echo "usage: $0 [auto|fresh|resume]" >&2; exit 2; fi
mode="${1:-auto}"
case "${mode}" in auto|fresh|resume) ;; *) echo "Unknown mode: ${mode}" >&2; exit 2 ;; esac
cd "$(dirname "$0")/../../.."
...
if [[ -d "${B0_10K_OUTPUT}" && -n "$(ls -A "${B0_10K_OUTPUT}")" && "${B0_10K_RESUME_FROM:-}" != "${B0_10K_OUTPUT}" ]]; then
  if [[ "${mode}" != auto || ! -f "${B0_10K_OUTPUT}/group_summary.json" ]]; then
    echo 'Nonempty output: choose a new B0_10K_OUTPUT or resume this group in place' >&2
    exit 1
  fi
fi
