#!/usr/bin/env python3
"""실제 가중치와 합성 입력으로 새 학습 루프를 점검한다. 논문 결과가 아니다."""
import argparse
import gc
import json
import os
...
from ibkd_seg.cityscapes.b0.losses import FSKD
...
def main():
    ...
    try:
        for method in ('vanilla','fskd','lg','alg','ibkd_lambda025','ibkd_lambda050'):
            ...
            engine=Engine(student,None if method=='vanilla' else teacher,guide,method,.1,torch.device('cpu'),
                          dict(batch_size=2,synthetic=True,method=method),args.output/method)
            seed_all(200001)
            ...
            row=dict(method=method,status=status,completed_steps=6,last_loss=engine.progress['last_loss'],
                     ...
                     inline_replay=engine.progress['inline_replay'],teacher_frozen_verified=method!='vanilla')
