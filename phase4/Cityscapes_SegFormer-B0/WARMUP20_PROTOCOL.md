...
Warm-up 20의 성능 개선이나 최종 β는 미확정입니다. 이후 사용자 결정으로 λ=0.25는
**β=0.387021·1.9351**만 10k에 유지하고, 이번에는 1위 β=0.387021과 ALG 1위 β=0.197479를
[한 이슈](H200_TOP1_10K_ISSUE.md)에서 실행해 [2/2 완료](reports/h200_top1_10k_v1/RESULTS.md)했습니다.
iBKD는 20번째 관측인 step3720에서 종료 조건을 만족했고 보호 검사를 통과했습니다.
ALG는 보호 0을 유지하며 step2418에서 종료했습니다. 두 번째 후보들의 10k가 남았습니다.
...
별도로 마련했습니다. λ=0.25의 smoke·2k 및 1위 β의 10k는 완료했습니다.
λ=0.25의 2순위 β 10k와 λ=0.5 warm-up 20은 아직 미실행입니다.
...
## 2026-09-25 최신 후보 실행 범위

[실행 범위 JSON](configs/b0_top2_10k_v1.json)에 사용자 후속 결정을 기록했습니다.
λ=0.25는 2k 상위 두 후보만 비교하며 두 후보 모두 10k를 완료한 뒤 최종 β 하나를 선택합니다.
이번에는 각 방법의 1위만 실행하므로 최종 선택은 보류합니다. λ=0.5 계획은 변경하지 않습니다.
아래 9월 24일의 네 후보 계획과 고정 JSON은 과거 결정·checkpoint 식별 보존용입니다.
학습 코드·손실·controller·입력·평가는 그대로이므로 같은 warm-up 20의 2k 전체 상태에서 이어갈 수 있습니다.
현재 전체 계획은 기존 6개 + λ=0.25 두 개 + λ=0.5 네 개 = 12개 궤적입니다.

## 2026-09-24 재선별 계획 기록
...
당시 계획의 10k 대상은 **기존 6개 + 새로운 iBKD 8개 = 14개 실행 궤적**입니다.
...
λ=0.25의 2k와 1위 β의 10k 결과를 기록했습니다. 실제 3720 보호 종료 및 이후 CE-only 학습까지 확인했습니다.
