...
- 상태: **공통 프로토콜 고정 / smoke CPU 검사 통과 / GPU·전체 학습 runner 전**
...
수치 β 선별과 GPU 검증·전체 학습 runner는 남아 있습니다.
가중치 적재 및 B0 smoke 연결·CPU 재개 검사는 완료했습니다.
...
역변환합니다. CIRKD Baidu 원본과의 동일성은 미확인이므로 본실험 초기화 확정과 구분합니다.
공통 JSON의 원래 해시는 유지하며, 예외는 [smoke 명세](configs/b0_smoke_v1.json)에 둡니다.
3 update·val2·checkpoint 재실행을 수행하는 runner를 구현했고 CPU 연결 검사는 통과했습니다.
[H200 이슈](H200_SMOKE_ISSUE.md)에서 실제 batch·메모리·재개·입력 동일성을 확인합니다.
