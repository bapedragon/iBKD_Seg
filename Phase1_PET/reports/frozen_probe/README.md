...
```bash
PYTHONPATH=src python Phase1_PET/scripts/import_probe_archive.py \
  /path/to/result.zip \
  --batch-size 128 \
  --issue-id ISSUE_ID \
  --output-dir Phase1_PET/results/raw/oxford_iiit_pet/frozen_probe_v1/batch128 \
...
```bash
PYTHONPATH=src python Phase1_PET/scripts/curate_probe_results.py \
  --raw-dir Phase1_PET/results/raw/oxford_iiit_pet/frozen_probe_v1/batch128 \
  --report-dir Phase1_PET/reports/frozen_probe/batch128
```
