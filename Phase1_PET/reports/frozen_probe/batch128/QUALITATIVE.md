# Phase 1 Pet batch 128 frozen probe 정성 결과
...
육안 확인에서 파일 손상, 열 순서 오류, 빈 panel 또는 전면 단색 출력 같은 명백한
파이프라인 실패는 보이지 않았습니다. 사례별 예측은 입력 배경까지 넓게 활성화되는
경우가 있어 정성 그림만으로 우열을 정하면 안 됩니다. 이 8개는 사전에 고정한 설명용
사례이고, 방법 순위는 3,669개 test 전체의 global confusion metric을 기준으로 합니다.
