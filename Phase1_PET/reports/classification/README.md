...
`Phase1_PET/results/raw/` 아래에 보관하며 `.gitignore`로 제외합니다. 원본 ZIP은 CRC와
...
```bash
PYTHONPATH=src python Phase1_PET/scripts/import_classification_archive.py \
  /path/to/result.zip \
  --batch-size 128 \
  --issue-id 702 \
  --output-dir Phase1_PET/results/raw/oxford_iiit_pet/full_classification_v1/batch128 \
...
```bash
PYTHONPATH=src python Phase1_PET/scripts/curate_classification_results.py \
  --raw-dir Phase1_PET/results/raw/oxford_iiit_pet/full_classification_v1/batch128 \
  --report-dir Phase1_PET/reports/classification/batch128
```
