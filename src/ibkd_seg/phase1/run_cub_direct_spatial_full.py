    ...
    for probe_seed in protocol["probe_seeds"]:
        ...
        for learning_rate in protocol["learning_rates"]:
            ...
            history_relative = (
                ...
                / (
                    f"{variant}_probe_seed{probe_seed}_"
                    f"lr{_learning_rate_slug(float(learning_rate))}.json"
                )
            )
            _atomic_json_save(
                {
            ...
            log(
                ...
                f"variant={variant} encoder_seed=1 probe_seed={probe_seed} "
        ...
        checkpoint_relative = (
            ...
            / f"{variant}_encoder_seed1_probe_seed{probe_seed}_best_validation.pt"
        )
    ...
    *,
    ...
    for selection in selections:
        ...
        log(
            ...
            f"variant={variant} encoder_seed=1 "
            ...
            if record.image_id in qualitative_ids:
                save_attention_triptych(
                    ...
                    destination=(
                        ...
                        / f"{variant}_encoder_seed1_image{record.image_id}.png"
                    ),
                )
...
def _part_aggregates(test_rows: Sequence[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for variant in EXPECTED_VARIANTS:
        ...
        if [row["probe_seed"] for row in rows] != [1, 2, 3, 4, 5]:
            raise RuntimeError(f"part-probe seed inventory mismatch: {variant}")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for index, item in enumerate(metric_config["checkpoint_inputs"], 1):
        ...
        student, audit = _load_student(student_root, item, device=device)
    ...
    _atomic_json_save(
        {
            ...
            "part_probe_validation_selections_complete": 20,
    ...
    for item in metric_config["checkpoint_inputs"]:
        variant = item["variant"]
        student, _audit = _load_student(student_root, item, device=device)
        variant_selections = [
            selection
            for selection in selections
    ...
    part_lookup = {row["variant"]: row for row in aggregates}
    cka_lookup = {
        (row["variant"], row["student_block"]): row for row in cka_rows
    }
    ...
    log("")
    log("[DIRECT_SPATIAL_FULL_SEED1_RESULTS]")
    for variant in EXPECTED_VARIANTS:
        log(
            f"[DIRECT_SPATIAL_FULL_RESULT] variant={variant} encoder_seed=1 "
            "part_test_pck_probe_seed_mean="
            f"{part_lookup[variant]['test_micro_pck_at_0.1_probe_seed_mean']:.6f} "
            "part_test_pck_probe_seed_sd="
            f"{part_lookup[variant]['test_micro_pck_at_0.1_probe_seed_sample_sd']:.6f} "
            "cka_block11="
            f"{cka_lookup[(variant, 11)]['centered_linear_cka']:.6f} "
            "attention_patch_ap="
            f"{attention_lookup[variant]['global_micro_patch_average_precision']:.6f} "
            "attention_pointing="
            f"{attention_lookup[variant]['pointing_game_peak_inside_mask']:.6f} "
            "attention_foreground_mass="
            f"{attention_lookup[variant]['foreground_attention_mass_mean']:.6f}"
        )
    log(
        "[DIRECT_SPATIAL_FULL_SEED1_DONE] status=complete encoder_seed=1 "
