"""Evaluate one verified 500-step Tiny student on the full Cityscapes val split."""
...
def terminal_report(report):
    ...
    result['score_scope'] = '500-step checkpoint full-val timing probe; not a 2000-step result or beta ranking'
...
def main():
    ...
    try:
        ...
        report['phase_seconds']['checkpoint_preflight'] = time.perf_counter() - started
        save_json(output / 'checkpoint_preflight.json', {k:source[k] for k in
                  ('pointer', 'checkpoint', 'bytes', 'sha256', 'signature_checks')})
        if args.preflight_only:
            print('[TI16_VAL500_PREFLIGHT] checkpoint=500 signature=passed bytes_sha256=passed data_manifest=passed', flush=True)
            return
        ...
        if provenance['weights'] != source['identity']['assets']:
        ...
        with warnings.catch_warnings(record=True) as records:
            ...
            try:
                ...
                if sha(Path(source['checkpoint'])) != source['sha256']:
        ...
        if not args.preflight_only or failed:
