...
def _cpu_deformable_spatial(spatial: nn.Module, inputs: torch.Tensor) -> torch.Tensor:
    """Run the same modulated deformable-convolution equation on one CPU thread."""
    import torchvision
...
class DiagnosticCBAM(nn.Module):
    """Select original or deterministic-equivalent CBAM components for forensics."""
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if self.channel_mode == "adaptive":
            ...
            # and first-index tie gradient, while avoiding its nondeterministic
