...
data_dir="${CUB_SEGMENTATION_DATA_DIR:-/app/scratch/cub_segmentation_spatial_full_v1/data}"
...
output_root="${CUB_SEGMENTATION_SPATIAL_FULL_OUTPUT_DIR:-/app/output/cub_segmentation_spatial_diagnostics_seed1_full_v1}"
manifest="phase1/phase1_cub_Seg/reports/window30_seed1_v1/checkpoint_release.json"
config="phase1/phase1_cub_Seg/configs/spatial_diagnostics_seed1_full_v1.json"
mkdir -p "${output_root}/setup"
log_id="$(date -u +%Y%m%dT%H%M%SZ)_$$"

run_suite() {
  echo "[CUB_SEG_SPATIAL_FULL_SETUP] stage=environment"
  ...
  python -u -m ibkd_seg.cub_segmentation.spatial_diagnostics_full \
    --full \
    --config "${config}" \
