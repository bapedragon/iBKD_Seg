# Phase 1 Oxford-IIIT Pet batch 64 full classification 요청
...
```text
[Request]: 박철현 Phase1 Pet batch64 6설정 x 3seed full classification
```
...
```text
bash phase1/scripts/run_full_b64.sh
```
...
```text
Oxford-IIIT Pet Phase 1의 batch-size 64 전체 37-way 품종 분류 요청입니다.
...
- student: scratch DeiT-Tiny/16, batch 64, 300 epochs, FP32
...
- 결과가 좋은 lambda나 batch만 사후 선택하지 않으며 batch64 profile의 여섯 설정을 전부 보고
...
- 출력 루트: /app/output/phase1_pet_full_b64_v1

timing 로그 기반 순수 학습 환산시간은 8h 39m 27s입니다. 최초 설치/다운로드, dataset audit, checkpoint당 1회 test 시간이 추가되므로 600분으로 요청합니다.
...
```text
[TASK_COUNT] teacher=1 student=18 total=19 matrix=6_variants_x_3_seeds batch=64
...
[SEQUENCE_DONE] status=complete completed=19/19 failed=0 summary=/app/output/phase1_pet_full_b64_v1/classification_summary.json
```

## 결과로 보존할 파일

```text
/app/output/phase1_pet_full_b64_v1/dataset_audit.json
/app/output/phase1_pet_full_b64_v1/classification_summary.json
/app/output/phase1_pet_full_b64_v1/classification_summary.csv
/app/output/phase1_pet_full_b64_v1/sequence_status.json
/app/output/phase1_pet_full_b64_v1/teacher/*/teacher_best_validation.pt
/app/output/phase1_pet_full_b64_v1/teacher/*/summary.json
/app/output/phase1_pet_full_b64_v1/students/*/student_best_validation.pt
/app/output/phase1_pet_full_b64_v1/students/*/summary.json
```

batch 128 이슈 결과와 함께 회수한 뒤 두
